...
# M-One Personal Assistant — Version 2.4 — Interface Listening
# Built by Mahdi Rabiee
# Created: Sunday, May 8, 2022
# Update: Friday, June 24, 2022
...
while True:
    ...
    if 'help' in query or '?' in query:
        ...
        print("'in what language are you written'")
        ...
        print("'open calculator / calculator'")
        print("'open disk c / open drive c'")
        print("'open disk d / open drive d'")
        print("'open disk e / open drive e'")
        print("'open task manager / open task / task manager'")
        print("'new folder'")
        print("'del folder'")
        print("'del file'")
        ...
        print("'capslock / toggle capslock'")
        print("'numlock / toggle numlock'")
        print("'start menu'")
    ...
    if 'who am i' in query:
        print(f"Admin ({getUser})")
        speak(f"Admin ({getUser})")
        continue
    ...
    if 'in what language are you written' in query:
        print("I'm programmed in Python")
        speak("I'm programmed in Python")
        continue

    if 'do you know me' in query:
        listSentencesC = [f'You told me your name was {getUser}. I could never forget that 😊' , f'I remember you telling me your name was {getUser}']
        randomListSentencesC = random.choice(listSentencesA)
        print(randomListSentencesC)
        speak(randomListSentencesC)
        continue
    ...
    if 'change voice' in query:
        inpSet = input("Select Voice (Male (M) - Female (F)): ").lower()
        if inpSet == "male" or inpSet == "m":
            engine.setProperty('voice', voices[0].id)
            print("Voice successfully changed.")
            speak("Voice successfully changed.")
        if inpSet == "female" or inpSet == "f":
    ...
    if 'set alarm' in query or 'set a alarm' in query:
        speak("Set Alarm, Please set the clock and minutes alert.")

        dateDay = datetime.datetime.today()
        inpHour = int(input("Hour: "))
        inpMinute = int(input("Minute: "))
        dateSetAlarm = dateDay.replace(day=dateDay.day, hour=inpHour, minute=inpMinute, second=0, microsecond=0)
        date = dateSetAlarm-dateDay
        second = date.seconds

        def startAlarm():
            addressSoundAlarm = "alarm\\Alarm02.wav" # Now please upload your musics to the 'alarm' folder.
            while True:
                ws.PlaySound(addressSoundAlarm, ws.SND_FILENAME)
                ws.PlaySound(addressSoundAlarm, ws.SND_FILENAME)
                ws.PlaySound(addressSoundAlarm, ws.SND_FILENAME)
                ws.PlaySound(addressSoundAlarm, ws.SND_FILENAME)
                ws.PlaySound(addressSoundAlarm, ws.SND_FILENAME)
    ...
    if 'edit note' in query:
        file = open("notes.txt", "r")
        print("Text Note:", file.read(), "\n")
        notes = input("Edit Note: ")
        file = open("notes.txt", "a")
        file.write(notes)
        file.close()
        print("Your note was successfully edited.")
        speak("Your note was successfully edited.")
        continue
    ...
    if 'open calculator' in query or 'calculator' in query:
        print("Calculator is open now")
        speak("Calculator is open now")
        os.system("start calc")
        continue

    if 'open disk c' in query or 'open drive c' in query:
        print("Disk C is open now")
        speak("Disk C is open now")
        os.system("start C:\\")
        continue
    if 'open disk d' in query or 'open drive d' in query:
        print("Disk D is open now")
        speak("Disk D is open now")
        os.system("start D:\\")
        continue
    if 'open disk e' in query or 'open drive e' in query:
        print("Disk E is open now")
        speak("Disk E is open now")
        os.system("start E:\\")
        continue

    if 'open task manager' in query or 'open task' in query or 'task manager' in query:
        print("Task Manager is open now")
        speak("Task Manager is open now")
    ...
    if 'capslock' in query or 'toggle capslock' in query:
        print("Capslock (Toggle)")
        speak("Capslock")
        pyautogui.press("CAPSLOCK")
        continue
    
    if 'numlock' in query or 'toggle numlock' in query:
        print("Numlock (Toggle)")
        speak("Numlock")
        pyautogui.press("NUMLOCK")
        continue

    if 'start menu' in query:
        print("Start Menu")
        speak("Start Menu")
        pyautogui.press("WIN")
        continue
        ...
        listSentencesNotFound = ['Not Found.' , "Sorry, I didn't understand."]
        randomListSentencesNotFound = random.choice(listSentencesNotFound)
        print(randomListSentencesNotFound)
        speak(randomListSentencesNotFound)